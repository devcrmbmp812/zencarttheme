 ...
 $total_record = $iInfo->RecordCount();
	...
	FROM " . TABLE_PRODUCTS_ATTRIBUTES  ." a INNER JOIN " . TABLE_PRODUCTS_OPTIONS  . " po ON po.products_options_id = a.options_id INNER JOIN " . TABLE_PRODUCTS_OPTIONS_VALUES  . "  pov ON pov.products_options_values_id = a.options_values_id AND pov.language_id = '1' AND po.language_id = '1' AND a.products_id =".$iInfo->fields['products_id']);
			...
			$ioInfo->MoveNext();
			 ...
			 $attribute_iInfo = $db->Execute("SELECT DISTINCT  ". TABLE_PRODUCTS_ATTRIBUTES .".products_attributes_id, ". TABLE_PRODUCTS_OPTIONS_VALUES .".products_options_values_name AS attribute_sku, ". TABLE_PRODUCTS_DESCRIPTION . ".products_name,". TABLE_PRODUCTS .". products_quantity AS attribute_quantity,(". TABLE_PRODUCTS .".products_price + ". TABLE_PRODUCTS_ATTRIBUTES .".options_values_price ) AS attribute_price FROM ". TABLE_PRODUCTS_ATTRIBUTES ."
			INNER JOIN ". TABLE_PRODUCTS_OPTIONS. " ON ". TABLE_PRODUCTS_ATTRIBUTES .".options_id = ". TABLE_PRODUCTS_OPTIONS . ".products_options_id 
			INNER JOIN ". TABLE_PRODUCTS_DESCRIPTION. " ON ". TABLE_PRODUCTS_DESCRIPTION .".products_id = ". TABLE_PRODUCTS_ATTRIBUTES . ".products_id 
			INNER JOIN ". TABLE_PRODUCTS. " ON ". TABLE_PRODUCTS .".products_id = ". TABLE_PRODUCTS_ATTRIBUTES . ".products_id 
			LEFT JOIN " . TABLE_PRODUCTS_OPTIONS_VALUES . " ON  " . TABLE_PRODUCTS_ATTRIBUTES . ".options_values_id =  ". TABLE_PRODUCTS_OPTIONS_VALUES .".products_options_values_id WHERE ". TABLE_PRODUCTS_ATTRIBUTES . ".products_id =".$iInfo->fields['products_id']."");
            
             while (!$attribute_iInfo->EOF) 
			{
			
              $Variant = new WG_Variant();
              $Variant->setItemCode($attribute_iInfo->fields['attribute_sku']);
              $Variant->setVarientID($attribute_iInfo->fields['products_attributes_id']);
			  $Variant->setQuantity($attribute_iInfo->fields['attribute_quantity']);
			  $Variant->setUnitPrice((float)$attribute_iInfo->fields['attribute_price']);
		      $Variant->setStatus("Success");
		      $attribute_iInfo->MoveNext();
