...
<script type="text/javascript">
function displaydefaultsettingfields(radioObj) {
  var radioLength = radioObj.length;
  
  if (radioObj.value=='Calculated Shipping (based on item weight)' || radioObj.value=='Free Domestic Shipping' && radioObj.checked==true) { 
    document.getElementById('defaultfixedfee').style.display='none';
    document.getElementById('defaultpackage').style.display='block';
  }else if (radioObj.value=='Fixed Fee Shipping' && radioObj.checked==true) { 
    document.getElementById('defaultpackage').style.display='none';
    document.getElementById('defaultfixedfee').style.display='block';
  }
}
</script>
</head>
...
<?php
  ...
  switch ($action) {
    ...
    case 'edit':
      ...
      $keys = '';
      reset($mInfo->keys);
      // START AuctionInc While loop
      while (list($key, $value) = each($mInfo->keys)) {

        if($value['set_function'] && $key=='MODULE_SHIPPING_AUCTIONINC_DEFAULTCALCMETHOD') { 
        $calcmethod_c='';
        $calcmethod_f='';
        $calcmethod_ci='';
        $calcmethod_c_status='none';
        $calcmethod_f_status='none';
        $calcmethod_ci_status='none';
        
        if($value['value']=='Calculated Shipping (based on item weight)') {
          $calcmethod_c='checked';
          $calcmethod_c_status='block';
        } 
        else if($value['value']=='Fixed Fee Shipping') {
          $calcmethod_f='checked';
          $calcmethod_f_status='block'; 
        }
        else if($value['value']=='Free Domestic Shipping') {
          $calcmethod_ci='checked';
          $calcmethod_ci_status='block'; 
        }
          
        $keys .= '<b>' . $value['title'] . '</b><br>' . $value['description'] . '<br>';
        //$keys .="<input type='radio' id='calculated shipping (based on item weight)-configuration[module_shipping_auctioninc_defaultcalcmethod]' value='Calculated Shipping (based on item weight)' name='configuration[MODULE_SHIPPING_AUCTIONINC_DEFAULTCALCMETHOD]' onClick='displaydefaultsettingfields(this)' ".$calcmethod_c."><label class='inputSelect' for='calculated shipping (based on item weight)-configuration[module_shipping_auctioninc_defaultcalcmethod]'>Calculated Shipping (based on item weight)</label><br><input type='radio' id='fixed fee shipping-configuration[module_shipping_auctioninc_defaultcalcmethod]' value='Fixed Fee Shipping' name='configuration[MODULE_SHIPPING_AUCTIONINC_DEFAULTCALCMETHOD]' onClick='displaydefaultsettingfields(this)' ".$calcmethod_f."><label class='inputSelect' for='fixed fee shipping-configuration[module_shipping_auctioninc_defaultcalcmethod]'>Fixed Fee Shipping</label>";
        // -- FREE DOMESTIC -- REPLACE THE ABOVE LINE WITH THE BELOW LINE
        $keys .="<input type='radio' id='calculated shipping (based on item weight)-configuration[module_shipping_auctioninc_defaultcalcmethod]' value='Calculated Shipping (based on item weight)' name='configuration[MODULE_SHIPPING_AUCTIONINC_DEFAULTCALCMETHOD]' onClick='displaydefaultsettingfields(this)' ".$calcmethod_c."><label class='inputSelect' for='calculated shipping (based on item weight)-configuration[module_shipping_auctioninc_defaultcalcmethod]'>Calculated Shipping (based on item weight)</label><br><input type='radio' id='fixed fee shipping-configuration[module_shipping_auctioninc_defaultcalcmethod]' value='Fixed Fee Shipping' name='configuration[MODULE_SHIPPING_AUCTIONINC_DEFAULTCALCMETHOD]' onClick='displaydefaultsettingfields(this)' ".$calcmethod_f."><label class='inputSelect' for='fixed fee shipping-configuration[module_shipping_auctioninc_defaultcalcmethod]'>Fixed Fee Shipping</label><br><input type='radio' id='free domestic shipping-configuration[module_shipping_auctioninc_defaultcalcmethod]' value='Free Domestic Shipping' name='configuration[MODULE_SHIPPING_AUCTIONINC_DEFAULTCALCMETHOD]' onClick='displaydefaultsettingfields(this)' ".$calcmethod_ci."><label class='inputSelect' for='free domestic shipping-configuration[module_shipping_auctioninc_defaultcalcmethod]'>Free Domestic Shipping</label>";        
        } 
        else if($key=='MODULE_SHIPPING_AUCTIONINC_DEFAULTPACKAGE') {
        
        if($value['value']=='Together with other items')
          $package_t='checked';
        else if($value['value']=='Separately')
          $package_s='checked';
        
        $keys .= '<div id="defaultpackage" style="display:'.$calcmethod_c_status.';"><b>' . $value['title'] . '</b><br>' . $value['description'] . '<br>';
        $keys .="<input type='radio' id='together with other items-configuration[module_shipping_auctioninc_defaultpackage]' value='Together with other items' name='configuration[MODULE_SHIPPING_AUCTIONINC_DEFAULTPACKAGE]' ".$package_t."><label class='inputSelect' for='together with other items-configuration[module_shipping_auctioninc_defaultpackage]'>Together with other items</label><br><input type='radio' id='separately-configuration[module_shipping_auctioninc_defaultpackage]' value='Separately' name='configuration[MODULE_SHIPPING_AUCTIONINC_DEFAULTPACKAGE]' ".$package_s."><label class='inputSelect' for='separately-configuration[module_shipping_auctioninc_defaultpackage]'>Separately</label></div>";
        } 
        else if($key=='MODULE_SHIPPING_AUCTIONINC_DEFAULTFIXEDFEE') { 
        $keys .= '<div id="defaultfixedfee" style="display:'.$calcmethod_f_status.';"><b>' . $value['title'] . '</b><br>' . $value['description'] . '<br>';
        $keys .="<input type='text' name='configuration[MODULE_SHIPPING_AUCTIONINC_DEFAULTFIXEDFEE]' value=".$value['value']."></div>";
        }      
        else {    
           $keys .= '<b>' . $value['title'] . '</b><br>' . $value['description'] . '<br>';               
