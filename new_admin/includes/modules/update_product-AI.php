 ...
 */
  ...
  if (isset($_POST['edit_x']) || isset($_POST['edit_y'])) {
    ...
    $sql_data_array = array('products_quantity' => $products_quantity,
      ...
      //  START => If the AuctionInc Module is enabled(On) store the auctioninc specific values into the array
      if(MODULE_SHIPPING_AUCTIONINC_STATUS=='On') {

        if($_POST['actioninc_insurable'])
           $actioninc_insurable=$_POST['actioninc_insurable'];
        else
           $actioninc_insurable='No';
          
        if($_POST['actioninc_handlingcode']!='Handling code')
           $actioninc_handlingcode=$_POST['actioninc_handlingcode'];
        else
           $actioninc_handlingcode='';
          
        if($_POST['actioninc_origincode']!='default')
           $actioninc_origincode=$_POST['actioninc_origincode'];
        else
           $actioninc_origincode='';  
          
        $sql_data_array_auctioninc  = 
      array('actioninc_calcmethod' => zen_db_prepare_input($_POST['actioninc_calcmethod']),
        'actioninc_length' => zen_db_prepare_input($_POST['actioninc_length']),
        'actioninc_width' => zen_db_prepare_input($_POST['actioninc_width']),
        'actioninc_height' => zen_db_prepare_input($_POST['actioninc_height']),
        'actioninc_package' => zen_db_prepare_input($_POST['actioninc_package']),
        'actioninc_origincode' => zen_db_prepare_input($actioninc_origincode),
        'actioninc_insurable' => zen_db_prepare_input($actioninc_insurable),
        'actioninc_handlingcode' => zen_db_prepare_input($actioninc_handlingcode),
        'actioninc_fixeddollaramount' => zen_db_prepare_input($_POST['actioninc_fixeddollaramount']),
        'actioninc_servicecodes' => zen_db_prepare_input($_POST['actioninc_servicecodes']),
        'actioninc_specialaccessorialfees' => zen_db_prepare_input($_POST['actioninc_specialaccessorialfees']),
        'actioninc_fixedfeecode' => zen_db_prepare_input($_POST['actioninc_fixedfeecode']),
        'actioninc_fixedfee1' => zen_db_prepare_input($_POST['actioninc_fixedfee1']),
        'actioninc_fixedfee2' => zen_db_prepare_input($_POST['actioninc_fixedfee2'])
                     );
                    
          $sql_data_array = array_merge($sql_data_array, $sql_data_array_auctioninc);
     }
     // END Auctioninc Module
