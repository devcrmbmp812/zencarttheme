...
// verify the selected shipping address
  ...
  // Verify sendto not in AUS/NZ if using a savings code  
  if (isset($_SESSION['savings_code'])) {
     $check_country_query = "SELECT entry_country_id FROM " . TABLE_ADDRESS_BOOK . " WHERE address_book_id = :addressBookID";

     $check_country_query = $db->bindVars($check_country_query, ':addressBookID', $_SESSION['sendto'], 'integer');
     $check_country = $db->Execute($check_country_query);
     $country = $check_country->fields['entry_country_id']; 
     if ($country == 13 || $country == 153) { 
        $messageStack->add_session('shopping_cart', 'Savings codes cannot be used when shipping to AUS/NZ', 'success');
        unset($_SESSION['savings_code']); 
        zen_redirect(zen_href_link(FILENAME_SHOPPING_CART));
     }
  }
