...
*/
    ...
    for ($i=0, $n=sizeof($products); $i<$n; $i++) {
      ...
      $this->products[$index] = array('qty' => $products[$i]['quantity'],
                                      ...
                                      'id' => $products[$i]['id'],
                                      'rowClass' => $rowClass);
      if (MODULE_SHIPPING_AUCTIONINC_STATUS=='On') {
         $this->products[$index]['quantity'] = $products[$i]['quantity'];
         $this->products[$index]['actioninc_calcmethod'] = $products[$i]['actioninc_calcmethod'];
         $this->products[$index]['actioninc_length'] = $products[$i]['actioninc_length'];
         $this->products[$index]['actioninc_width'] = $products[$i]['actioninc_width'];
         $this->products[$index]['actioninc_height'] = $products[$i]['actioninc_height'];
         $this->products[$index]['actioninc_package'] = $products[$i]['actioninc_package'];
         $this->products[$index]['actioninc_origincode'] = $products[$i]['actioninc_origincode'];
         $this->products[$index]['actioninc_insurable'] = $products[$i]['actioninc_insurable'];
         $this->products[$index]['actioninc_handlingcode'] = $products[$i]['actioninc_handlingcode'];
         $this->products[$index]['actioninc_fixeddollaramount'] = $products[$i]['actioninc_fixeddollaramount'];
         $this->products[$index]['actioninc_servicecodes'] = $products[$i]['actioninc_servicecodes'];
         $this->products[$index]['actioninc_specialaccessorialfees'] = $products[$i]['actioninc_specialaccessorialfees'];
         $this->products[$index]['actioninc_fixedfeecode'] = $products[$i]['actioninc_fixedfeecode'];
         $this->products[$index]['actioninc_fixedfee1'] = $products[$i]['actioninc_fixedfee1'];
         $this->products[$index]['actioninc_fixedfee2'] = $products[$i]['actioninc_fixedfee2'];
      }  
...
*/
  ...
  function create($zf_ot_modules, $zf_mode = 2) {
    ...
    $c='';
    foreach($package_details as $type => $packages) {
      if( ("Ship Service (" . $type . ")") == $this->info['shipping_method']) {
        $c=$type;
        break;
      } 
    }
    $packages = $package_details[$c];

    $cntVar1=1;
    $package_html = '';
    if(is_array($packages)) {
      foreach($packages as $package) {
        $package_html .= "<strong>Package #{$cntVar1}</strong><br>";
        $package_html .= "Flat Rate Code: " . $package['FlatRateCode'] . "<br>";
        $package_html .= "Quantity: " . $package['Quantity'] . "<br>";
        $package_html .= "Pack Method: " . $package['PackMethod'] . "<br>";
        $package_html .= "Origin: " . $package['Origin'] . "<br>";
        $package_html .= "Declared Value: " . $package['DeclaredValue'] . "<br>";
        $package_html .= "Weight: " . $package['Weight'] . "<br>";
        $package_html .= "Length: " . $package['Length'] . "<br>";
        $package_html .= "Width: " . $package['Width'] . "<br>";
        $package_html .= "Height: " . $package['Height'] . "<br>";
        $package_html .= "Oversize Code: " . $package['OversizeCode'] . "<br>";
        $package_html .= "Carrier Rate: " . $package['CarrierRate'] . "<br>";
        $package_html .= "Surcharge: " . $package['Surcharge'] . "<br>";
        $package_html .= "Fuel Surcharge: " . $package['FuelSurcharge'] . "<br>";
        $package_html .= "Insurance: " . $package['Insurance'] . "<br>";
        $package_html .= "Handling: " . $package['Handling'] . "<br>";
        $package_html .= "Total Rate: " . $package['ShipRate'] . "<br>"; 

        $cntVar2=1;
        foreach($package['Items'] as $item) {
          $package_html .= "<br><strong>Item #{$cntVar2}</strong><br>";
          $package_html .= "Ref Code: " . $item['RefCode'] . "<br>";
          $package_html .= "Quantity: " . $item['Qty'] . "<br>";
          $package_html .= "Weight: " . $item['Weight'] . "<br>";
          $cntVar2++;
        }          
        $package_html .= "<br><br>";
        $cntVar1++;
    ...
    $sql_data_array = array('customers_id' => $_SESSION['customer_id'],
                            ...
                            'ip_address' => $_SESSION['customers_ip_address'] . ' - ' . $_SERVER['REMOTE_ADDR'],
                            'actioninc_packages' => $package_html
                            );
